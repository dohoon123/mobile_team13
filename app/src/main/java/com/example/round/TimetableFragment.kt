        ...
        val simpleCallback=object:ItemTouchHelper.SimpleCallback(ItemTouchHelper.DOWN or ItemTouchHelper.UP, ItemTouchHelper.RIGHT){
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                builder.setPositiveButton("삭제", DialogInterface.OnClickListener { dialog, which ->
                    ...
                    var flag = sDBHelper.deleteScheduleByR(removeItem.routineID) //루틴 안의 스케줄 먼저 삭제
                    if(flag == true) {
                        flag=rDBHelper.deleteRoutine(removeItem) //루틴 삭제
                        if (flag == true){
                            Toast.makeText(activity, "루틴이 삭제되었습니다", Toast.LENGTH_SHORT).show()
